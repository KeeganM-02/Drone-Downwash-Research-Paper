# ==============================
# Backpack Sprayer Deposition Analysis (Separate)
# ==============================
# Author: Keegan Miller
# Description:
#   Generates and saves mean, standard deviation, and box-and-whisker plots
#   of coverage rate, density, and droplet count
#   grouped by canopy position for the Backpack Sprayer trials.
#   Includes separate analysis for (B)ottom and (T)op leaf surface data,
#   and exports numeric summaries to CSV files.

import pandas as pd
import matplotlib.pyplot as plt
import os

# === Global Font Size Settings ===
plt.rcParams.update({
    "font.size": 16,          # base font size
    "axes.titlesize": 18,     # title size
    "axes.labelsize": 16,     # axis label size
    "xtick.labelsize": 14,    # x-axis tick labels
    "ytick.labelsize": 14,    # y-axis tick labels
    "legend.fontsize": 14     # legend text
})

# === Load data ===
df = pd.read_csv("wsp_analysis_results.csv")

# === Filter for Backpack Sprayer trials only ===
df = df[df['Height'].str.contains('Backpack Sprayer', case=False, na=False)]

# === Extract the (B) or (T) marker as a new "Surface" column ===
df['Surface'] = df['Height'].str.extract(r'\((B|T)\)')[0].map({'B': 'Bottom', 'T': 'Top'})

# === Simplify the Height column ===
df['Height'] = df['Height'].str.extract(r'(Backpack Sprayer ?\([BT]\))')[0]

# === Ensure correct order for Height and Canopy Position ===
height_order = ['Backpack Sprayer(B)', 'Backpack Sprayer(T)']
canopy_order = ['Top', 'Mid', 'Bot']
 
df['Height'] = pd.Categorical(df['Height'], categories=height_order, ordered=True)
df['Canopy_Position'] = pd.Categorical(df['Canopy_Position'], categories=canopy_order, ordered=True)

# === Create output folders ===
output_dir = "plots_backpack_2"
os.makedirs(output_dir, exist_ok=True)

tables_dir = "tables_backpack"
os.makedirs(tables_dir, exist_ok=True)

# === Measurements to analyze ===
measurements = ['Coverage_Rate', 'Coverage_Density', 'Droplet_Count']

# === Pretty labels ===
pretty_labels = {
    'Coverage_Rate': 'Coverage Rate (%)',
    'Coverage_Density': 'Coverage Density',
    'Droplet_Count': 'Droplet Count',
    'Height': 'Application Method',
    'Canopy_Position': 'Canopy Position',
    'Surface': 'Leaf Surface'
}

# === Function to generate and save plots ===
def plot_stats(data, group_by, measurement):
    grouped = data.groupby(group_by, observed=True)[measurement].agg(['mean', 'std', 'count']).reset_index()

    y_label = pretty_labels[measurement]
    x_label = pretty_labels.get(group_by, group_by)

    # --- Mean Plot ---
    plt.figure(figsize=(8, 6))
    plt.bar(grouped[group_by], grouped['mean'], color='#0c5449')
    plt.title(f'{pretty_labels[measurement]} Mean by {x_label}')
    plt.xlabel(x_label)
    plt.ylabel(y_label)
    plt.grid(axis='y', linestyle='--', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{measurement}_Mean_by_{group_by}.png"), dpi=300)
    plt.close()

    # --- Standard Deviation Plot ---
    plt.figure(figsize=(8, 6))
    plt.bar(grouped[group_by], grouped['std'], color='#ffcc33')
    plt.title(f'{pretty_labels[measurement]} Standard Deviation by {x_label}')
    plt.xlabel(x_label)
    plt.ylabel(f'{y_label} Std Dev')
    plt.grid(axis='y', linestyle='--', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{measurement}_Std_by_{group_by}.png"), dpi=300)
    plt.close()

    # --- Box Plot (Full Range) ---
    plt.figure(figsize=(8, 6))
    data.boxplot(column=measurement, by=group_by, grid=False, patch_artist=True,
                 boxprops=dict(facecolor='lightgreen', color='green'),
                 medianprops=dict(color='darkred', linewidth=1.5))
    plt.title(f'{pretty_labels[measurement]} by {x_label} (Full Range)')
    plt.suptitle('')
    plt.xlabel(x_label)
    plt.ylabel(y_label)
    plt.grid(axis='y', linestyle='--', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{measurement}_Box_by_{group_by}_Full.png"), dpi=300)
    plt.close()

    # --- Box Plot (Zoomed / No Outliers) ---
    q1 = data[measurement].quantile(0.25)
    q3 = data[measurement].quantile(0.75)
    iqr = q3 - q1
    lower_lim = q1 - 1.5 * iqr
    upper_lim = q3 + 1.5 * iqr

    plt.figure(figsize=(8, 6))
    data.boxplot(column=measurement, by=group_by, grid=False, patch_artist=True,
                 boxprops=dict(facecolor='lightgreen', color='green'),
                 medianprops=dict(color='darkred', linewidth=1.5))
    plt.ylim(lower_lim, upper_lim)
    plt.title(f'{pretty_labels[measurement]} by {x_label} (Zoomed-In)')
    plt.suptitle('')
    plt.xlabel(x_label)
    plt.ylabel(y_label)
    plt.grid(axis='y', linestyle='--', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{measurement}_Box_by_{group_by}_Zoomed.png"), dpi=300)
    plt.close()

    grouped['Measurement'] = measurement
    grouped['Grouping'] = group_by
    return grouped


# === Interaction plot ===
def plot_interaction(data, measurement):
    grouped = data.groupby(['Height', 'Canopy_Position'], observed=True)[measurement].mean().reset_index()

    plt.figure(figsize=(8, 6))
    for canopy in canopy_order:
        subset = grouped[grouped['Canopy_Position'] == canopy]
        plt.plot(subset['Height'], subset[measurement], marker='o', linewidth=2, label=canopy)

    plt.title(f'{pretty_labels[measurement]} by Surface and Canopy Position')
    plt.xlabel(pretty_labels['Height'])
    plt.ylabel(pretty_labels[measurement])
    plt.legend(title='Canopy Position')
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{measurement}_Interaction_Surface_Canopy.png"), dpi=300)
    plt.close()

    stats = data.groupby(['Height', 'Canopy_Position'], observed=True)[measurement].agg(['mean', 'std', 'count']).reset_index()
    stats['Measurement'] = measurement
    stats['Grouping'] = 'Surface×Canopy_Position'
    return stats


# === Generate and save plots ===
all_height_stats = []
all_canopy_stats = []
all_interaction_stats = []

for measurement in measurements:
    h_stats = plot_stats(df, 'Height', measurement)
    all_height_stats.append(h_stats)

    c_stats = plot_stats(df, 'Canopy_Position', measurement)
    all_canopy_stats.append(c_stats)

    i_stats = plot_interaction(df, measurement)
    all_interaction_stats.append(i_stats)


# === Combine and export ===
height_stats_df = pd.concat(all_height_stats, ignore_index=True)
canopy_stats_df = pd.concat(all_canopy_stats, ignore_index=True)
interaction_stats_df = pd.concat(all_interaction_stats, ignore_index=True)

height_stats_df = height_stats_df[['Measurement', 'Grouping', 'Height', 'mean', 'std', 'count']]
canopy_stats_df = canopy_stats_df[['Measurement', 'Grouping', 'Canopy_Position', 'mean', 'std', 'count']]
interaction_stats_df = interaction_stats_df[['Measurement', 'Grouping', 'Height', 'Canopy_Position', 'mean', 'std', 'count']]

height_stats_df.to_csv(os.path.join(tables_dir, "summary_by_surface.csv"), index=False)
canopy_stats_df.to_csv(os.path.join(tables_dir, "summary_by_canopy.csv"), index=False)
interaction_stats_df.to_csv(os.path.join(tables_dir, "summary_by_surface_canopy.csv"), index=False)

combined = pd.concat(
    [
        height_stats_df.assign(Canopy_Position=pd.NA),
        canopy_stats_df.assign(Height=pd.NA),
        interaction_stats_df
    ],
    ignore_index=True
)
combined = combined[['Measurement', 'Grouping', 'Height', 'Canopy_Position', 'mean', 'std', 'count']]
combined.to_csv(os.path.join(tables_dir, "summary_stats_all_backpack.csv"), index=False)

print(f"Backpack Sprayer analysis complete. Plots saved in '{output_dir}', summaries in '{tables_dir}'.")

# === Aggregate across surfaces (Top + Bottom) — no plots ===
agg_tables_dir = tables_dir  # reuse "tables_backpack"; or set to a different folder if you prefer

agg_by_canopy_all = []
agg_overall_all = []

for m in measurements:
    # Aggregate across surfaces but keep canopy position (Top/Mid/Bot)
    by_canopy = (
        df.groupby('Canopy_Position', observed=True)[m]
          .agg(['mean', 'std', 'count'])
          .reset_index()
    )
    by_canopy['Measurement'] = m
    by_canopy['Grouping'] = 'Aggregate(B+T)'
    agg_by_canopy_all.append(by_canopy)

    # Overall aggregate across both surfaces and all canopy positions
    overall = (
        df[m].agg(['mean', 'std', 'count'])
          .to_frame().T  # make it a 1-row DataFrame
    )
    overall['Measurement'] = m
    overall['Grouping'] = 'Aggregate(B+T)'
    # keep consistent column order
    overall = overall[['Measurement', 'Grouping', 'mean', 'std', 'count']]
    agg_overall_all.append(overall)

# Concatenate and reorder columns nicely
agg_by_canopy_df = pd.concat(agg_by_canopy_all, ignore_index=True)
agg_by_canopy_df = agg_by_canopy_df[['Measurement', 'Grouping', 'Canopy_Position', 'mean', 'std', 'count']]

agg_overall_df = pd.concat(agg_overall_all, ignore_index=True)
# already in desired order: ['Measurement','Grouping','mean','std','count']

# Save CSVs
agg_by_canopy_df.to_csv(os.path.join(agg_tables_dir, "summary_aggregate_by_canopy.csv"), index=False)
agg_overall_df.to_csv(os.path.join(agg_tables_dir, "summary_aggregate_overall.csv"), index=False)

print("Aggregate (B+T) summaries saved:",
      os.path.join(agg_tables_dir, "summary_aggregate_by_canopy.csv"),
      "and",
      os.path.join(agg_tables_dir, "summary_aggregate_overall.csv"))

