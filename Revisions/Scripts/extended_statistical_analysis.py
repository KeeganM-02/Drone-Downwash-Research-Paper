...
def analyze_metric_by_height(df, metric_name, metric_column):
    """Analyze a specific metric (Coverage Rate, Density, or Droplet Count) by drone height"""
    ...
    summary_text.append(f"=== DRONE HEIGHT COMPARISONS: {metric_name} ===\n")
    ...
    for height1, height2 in comparisons:
        ...
        # Get metric values for each height
        group1 = drone_data[drone_data['Height'] == height1][metric_column].dropna().values
        group2 = drone_data[drone_data['Height'] == height2][metric_column].dropna().values
        ...
        if len(group1) > 0 and len(group2) > 0:
            ...
            # Add to summary text
            summary_text.append(f"  {height1}: Mean = {mean1:.4f}, SD = {std1:.4f} (n={len(group1)})")
            summary_text.append(f"  {height2}: Mean = {mean2:.4f}, SD = {std2:.4f} (n={len(group2)})")
            summary_text.append(f"  Mean difference: {mean_diff:.4f}")
...
def analyze_metric_by_canopy(df, metric_name, metric_column):
    """Analyze a specific metric by canopy position"""
    
    # Filter drone data only
    drone_data = df[~df['Height'].str.contains('Backpack', na=False)]
    
    summary_text = []
    summary_text.append(f"=== CANOPY POSITION COMPARISONS: {metric_name} (Drone Data) ===\n")
    
    # Get data by canopy position
    top_data = drone_data[drone_data['Canopy_Position'] == 'Top'][metric_column].dropna().values
    bot_data = drone_data[drone_data['Canopy_Position'] == 'Bot'][metric_column].dropna().values
    mid_data = drone_data[drone_data['Canopy_Position'] == 'Mid'][metric_column].dropna().values
    ...
    # Top vs Middle comparison
    if len(top_data) > 0 and len(mid_data) > 0:
        mean_top, std_top = np.mean(top_data), np.std(top_data, ddof=1)
        mean_mid, std_mid = np.mean(mid_data), np.std(mid_data, ddof=1)
        
        cohens_d = calculate_cohens_d(mid_data, top_data)
        effect_interpretation = interpret_effect_size(cohens_d)
        mean_diff, ci = calculate_confidence_interval(mid_data, top_data)
        t_stat, p_value = stats.ttest_ind(mid_data, top_data)
        ...
        result = {
            ...
            'Group_1': 'Middle',
            'Group_2': 'Top',
            'Group_1_Mean': mean_mid,
            'Group_1_SD': std_mid,
            'Group_1_n': len(mid_data),
            'Group_2_Mean': mean_top,
            'Group_2_SD': std_top,
            'Group_2_n': len(top_data),
            'Mean_Difference': mean_diff,
        ...
        summary_text.append("--- Top vs Middle Canopy Position ---")
        summary_text.append(f"  Top: Mean = {mean_top:.4f}, SD = {std_top:.4f} (n={len(top_data)})")
        summary_text.append(f"  Middle: Mean = {mean_mid:.4f}, SD = {std_mid:.4f} (n={len(mid_data)})")
        summary_text.append(f"  Mean difference (Top - Middle): {mean_diff:.4f}")
        summary_text.append(f"  95% CI: [{ci[0]:.4f}, {ci[1]:.4f}]")
        summary_text.append(f"  Cohen's d: {cohens_d:.3f} ({effect_interpretation} effect)")
        summary_text.append(f"  p-value: {p_value:.3f}")
        summary_text.append("")
    
    # Middle vs Bottom comparison
    if len(mid_data) > 0 and len(bot_data) > 0:
        mean_mid, std_mid = np.mean(mid_data), np.std(mid_data, ddof=1)
        mean_bot, std_bot = np.mean(bot_data), np.std(bot_data, ddof=1)
        
        cohens_d = calculate_cohens_d(bot_data, mid_data)
        effect_interpretation = interpret_effect_size(cohens_d)
        mean_diff, ci = calculate_confidence_interval(bot_data, mid_data)
        t_stat, p_value = stats.ttest_ind(bot_data, mid_data)
        ...
        result = {
            ...
            'Group_1_SD': std_bot,
            'Group_1_n': len(bot_data),
            'Group_2_Mean': mean_mid,
            'Group_2_SD': std_mid,
            'Group_2_n': len(mid_data),
            'Mean_Difference': mean_diff,
            'CI_Lower_95': ci[0],
            'CI_Upper_95': ci[1],
            'Cohens_d': cohens_d,
        ...
        summary_text.append("--- Middle vs Bottom Canopy Position ---")
        summary_text.append(f"  Middle: Mean = {mean_mid:.4f}, SD = {std_mid:.4f} (n={len(mid_data)})")
        summary_text.append(f"  Bottom: Mean = {mean_bot:.4f}, SD = {std_bot:.4f} (n={len(bot_data)})")
        summary_text.append(f"  Mean difference (Middle - Bottom): {mean_diff:.4f}")
        summary_text.append(f"  95% CI: [{ci[0]:.4f}, {ci[1]:.4f}]")
        summary_text.append(f"  Cohen's d: {cohens_d:.3f} ({effect_interpretation} effect)")
        summary_text.append(f"  p-value: {p_value:.3f}")
        summary_text.append("")
...
def main():
    ...
    all_summary_text = []
    all_summary_text.append("Comprehensive Statistical Analysis Summary")
    ...
    for metric_name, metric_column in metrics:
        # Height analysis
        height_results, height_summary = analyze_metric_by_height(df, metric_name, metric_column)
        all_results.extend(height_results)
        all_summary_text.extend(height_summary)
        
        # Canopy position analysis
        canopy_results, canopy_summary = analyze_metric_by_canopy(df, metric_name, metric_column)
    ...
    paper_summary = []
    paper_summary.append("\n=== SIGNIFICANT FINDINGS SUMMARY FOR YOUR PAPER ===")
    paper_summary.append("")
    ...
    txt_filename = 'Comprehensive_Stats_Summary.txt'
